import React from 'react';
import "../Custom2/Custom2.style.css";
import { Link } from 'react-router-dom';
import { Contact } from 'lucide-react';
import Hbtn from '../Hbtn'
import Backbtn from '../BackBtn'
// const teamMembers = [
//   {
//     name: "Pranav Belorker",
//     image: "https://github.com/Aditya04tech/Portfolio_Images/blob/main/Pranav.jpg?raw=true",
//     portfolio: "http://pranavbelorker.in/",
//   },
//   {
//     name: "ME",
//     image: "https://github.com/Aditya04tech/Portfolio_Images/blob/main/Pranav.jpg?raw=true",
//     portfolio: "http://pranavbelorker.in/",
//   },
// ];

// const TeamSection = () => {
//   return (
//     <>
//       <h2 className="section-title font-weight-bold mb-3">Our Team</h2>
//       <section className="unique-team-section-wrapper">
//         <div className="unique-team-members">
//           {teamMembers.map((member, index) => (
//             <div className="unique-team-member-card" key={index}>
//               <a href={member.portfolio} target="_blank" rel="noopener noreferrer">
//                 <img src={member.image} alt={member.name} loading="lazy" />
//                 <span className="unique-team-member-name">{member.name}</span>
//               </a>
//             </div>
//           ))}
//         </div>
//       </section>
//     </>
//   );
// };

const Project4 = () => {
  return (
    <>
    <Hbtn/>
    <Backbtn/>
    <div className="main-wrapper">
      <section className="cta-section theme-bg-light py-5">
        <div className="container single-col-max-width">
          <h2 className="heading text-center">Case Study: DAYA HOSPITAL</h2>
          <div className="project-intro text-center">
            <p className="mb-0 lead">
            Daya Hospital provides expert healthcare services with experienced doctors, modern facilities, and compassionate care for comprehensive medical needs.
            </p>
          </div>
        </div>
      </section>

      <section className="project px-3 py-5 p-md-5">
        <div className="container">
          <div className="project-meta theme-bg-light p-4">
            <div className="row g-4 g-lg-5">
              <div className="col-md-5 col-lg-4 col-xxl-3 d-none d-md-block">
                <img className="project-thumb img-fluid rounded" src={`${process.env.PUBLIC_URL}/images/project/dayah1.png`} alt="dayah1"/>
              </div>
              <div className="col-12 col-md-7 col-lg-8 col-xxl-9">
                <div className="client-info">
                  <h3 className="client-name font-weight-bold mb-4">DAYA HOSPITAL</h3>
                  <ul className="client-meta list-unstyled">
                    <li className="mb-2">
                      <i className="fas fa-industry fa-fw me-2" />
                      <strong>Industry:</strong> HELTH
                    </li>
                    <li className="mb-2">
                      <i className="fas fa-users fa-fw me-2" />
                      <strong>Size:</strong> 5
                    </li>
                    <li className="mb-2">
                      <strong>
                        <i className="fas fa-link fa-fw me-2" />
                        Website:
                      </strong>
                      <a className="theme-link" href="http://dayahospitals.com/">http://dayahospitals.com/</a>
                    </li>
                  </ul>
                  <div className="client-bio mb-4">
                  Daya Hospital offers compassionate care and advanced medical technology, ensuring exceptional treatment, personalized attention, and a commitment to your health.
                  </div>
                  <h4 className="subheading mb-3">Project Requirements</h4>
                  <ul className="mb-0">
                  <li className="mb-2">User-Friendly Website - A simple and intuitive interface with a red and white theme for easy navigation.</li>
<li className="mb-2">Animations & Visuals - Engaging animations enhance the user experience, making information accessible and appealing.</li>
<li className="mb-2">Service Detail Pages - Dedicated pages providing detailed information about healthcare services and specialties.</li>
<li className="mb-2">Custom Images - Professionally designed images representing healthcare services for a personalized, trustworthy experience.</li>
<li className="mb-2">WhatsApp Integration - Patients can easily click the "Contact on WhatsApp" button to connect with the hospital for inquiries and appointments.</li>

                  </ul>
                </div>
              </div>
            </div>
          </div>

          <div className="project-sections py-5">
            <div className="project-section mb-5">
              <h3 className="project-section-title mb-3">Project Overview</h3>
              <p>
              Daya Hospital is a compassionate healthcare platform committed to patient well-being. It features a user-friendly, red and white-themed interface, engaging visuals, and dedicated service detail pages. Patients can easily explore healthcare services and connect directly with the hospital through WhatsApp for inquiries, appointments, and support. The platform ensures accessible, personalized medical care with a focus on advanced technology and compassionate treatment.
              </p>
            </div>

            <div className="project-section mb-5">
              <h3 className="project-section-title">The Challenge</h3>
              <p>
              Developing Daya Hospital's platform presented several challenges, including designing a visually appealing red and white UI structure, creating relevant healthcare content, and sourcing suitable medical images. Ensuring smooth WhatsApp integration for patient inquiries and appointments while maintaining a user-friendly experience was a significant hurdle. Continuous refinement was necessary to create a well-structured, engaging, and fully functional healthcare platform that meets patient needs and supports effective communication.
              </p>
            </div>

            <div className="row mt-5">
              <div className="col-12 col-lg-6 mb-5">
                <img className="img-fluid rounded" src={`${process.env.PUBLIC_URL}/images/project/dayah2.png`} alt="dayah2" />
              </div>
              <div className="col-12 col-lg-6 mb-5">
                <img className="img-fluid rounded"src={`${process.env.PUBLIC_URL}/images/project/dayah3.png`} alt="dayah3" />
              </div>
            </div>

            
          </div>

          {/* Team Section */}
          {/* <TeamSection /> */}
        </div>
      </section>

      <section className="promo-section theme-bg-light py-5 text-center">
        <div className="container single-col-max-width">
          <h2 className="title">Want me to help with your project?</h2>
          <p>If you take on freelance work, you can use this section to prompt any potential clients to get in touch with you.</p>
          <div className="text-center">
            <Link className="btn btn-primary" to="/contact" >
              <i className="fas fa-paper-plane me-2" />Hire Me
            </Link>
          </div>
        </div>
      </section>
    </div>
  </>
  );
};

export default Project4;
